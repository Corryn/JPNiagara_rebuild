...
$ccListOBJ = new CC_List(); 
	...
	if (!empty($_REQUEST)) {
		...
		$postFields["email_address"] = $_REQUEST["Email"];
		$postFields["first_name"] = $_REQUEST["My_Firstname"];
		$postFields["last_name"] = $_REQUEST["My_Lastname"];
		$postFields["middle_name"] = $_REQUEST["MiddleName"];
		$postFields["company_name"] = $_REQUEST["Campground_Name"];
		$postFields["job_title"]= $_REQUEST["JobTitle"];
		$postFields["home_number"] = $_REQUEST["Tel1"].$_REQUEST["Tel2"].$_REQUEST["Tel3"];
		$postFields["work_number"] = $_REQUEST["WorkPhone"];
		$postFields["address_line_1"] = $_REQUEST["Address"];
		$postFields["address_line_2"] = $_REQUEST["Addr2"];
		$postFields["address_line_3"] = $_REQUEST["Addr3"];
		$postFields["city_name"] = $_REQUEST["City"];
		$postFields["state_code"] = $_REQUEST["State"];
		$postFields["state_name"] = $_REQUEST["StateName"];
		$postFields["country_code"] = $_REQUEST["Country"];
		$postFields["zip_code"] = $_REQUEST["ZipPostal"];
