...
Slider = (function() {
  ...
  Slider.prototype.appendPagination = function() {
    ...
    return this.pagination().on('click', 'a', function(e) {
      _this.to((jQuery(e.currentTarget)).data().index - 1);
      _this.stopAutoplay();
      return false;
    });
  };
  ...
  Slider.prototype.previousLink = function() {
    return this.jQuerypreviousLink || (this.jQuerypreviousLink = jQuery('<a />', {
  ...
  Slider.prototype.nextLink = function() {
    return this.jQuerynextLink || (this.jQuerynextLink = jQuery('<a />', {
  ...
  Slider.prototype.pagination = function() {
    ...
    if (!this.jQuerypagination) {
      this.jQuerypagination = jQuery('<ul />', {
        "class": this.options.paginationClass
      });
      _ref = this.slides;
      for (index = _i = 0, _len = _ref.length; _i < _len; index = ++_i) {
        slide = _ref[index];
        this.jQuerypagination.append("<li><a href='#' data-index='" + (index + 1) + "'>" + (index + 1) + "</a></li>");
      }
    }
    return this.jQuerypagination;
  };
  ...
  Slider.prototype.initSlides = function() {
    ...
    this.container.children().each(function(index, element) {
      return _this.slides.push(new Slide(jQuery(element), index, _this.options));
    });
...
jQuery(function() {
  jQuery.miniSlider = function(element, options) {
    this.defaults = {
      autoPlay: true,
      delay: 3000,
    ...
    this.init = function() {
      ...
      this.slider = new Slider(this.jQueryelement, this.settings);
  ...
  return jQuery.fn.miniSlider = function(options) {
    return this.each(function() {
      var plugin;
      if (void 0 === jQuery(this).data('miniSlider')) {
        plugin = new jQuery.miniSlider(this, options);
        return jQuery(this).data('miniSlider', plugin);
      }
    });
  };
});
